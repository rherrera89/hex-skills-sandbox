...
4. For METRIC: it **displays a single value, it does not aggregate.** A METRIC reads one column at one row (`valueColumn` at `valueRowIndex: 0`); `valueAggregate` is **rejected by the import API** (any value — `"SUM"`, `"Sum"` — 500s with an opaque "unknown API error", though it passes JSON-schema validation). So for a computed KPI, feed it a **1-row SQL** and point the METRIC at that. Two ways to make that 1-row cell:
   - **Warehouse-scalar query (CLI-friendly — default).** A normal SQL cell against the resolved connection, e.g. `SELECT SUM(<col>) AS TOTAL FROM <base>`. `hex cell create --data-connection-id …` mints it directly, and the §9 ratio/KPI exception in [`tableau-semantics.md`](tableau-semantics.md) already blesses a thin dedicated query here.
   - **Dataframe-SQL over the shared df** (`dataFrameCell: true`, `dataConnectionId: null`, e.g. `SELECT SUM(<col>) AS TOTAL FROM <shared_df>`) reuses the consolidated cell instead of re-querying the warehouse — **but `hex cell create` CANNOT mint this.** A connection-less dataframe-SQL cell created via `hex cell create` **ERRORs**; it must be **authored in YAML** (add it to `cells[]` in the export). Use this only when you're already editing YAML; otherwise take the warehouse-scalar path above.

   Then set the METRIC's `valueVariableName` = that 1-row dataframe, `valueColumn` = its column, `valueRowIndex: 0`, `valueAggregate: null`, + `displayFormat`. (Same companion-SQL move as ratio charts, and why the template ships `valueRowIndex: 0`.)
