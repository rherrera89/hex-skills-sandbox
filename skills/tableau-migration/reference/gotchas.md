...
- **Sweep ALL filter scopes.** Tableau filters live at four scopes:
  ...
  - *Relative-date / date-range filters:* translate the window deliberately (they include the current period, so offsets are easy to get off-by-one) and sanity-check the totals — full rules in [`tableau-semantics.md`](tableau-semantics.md) §2.
...
- **Dashboard objects aren't only worksheets.** A dashboard also has `<zone type-v2='…'>` objects — **image** (`bitmap`), **text**, **web page/iframe** (`web`), **button**, **blank**. Don't drop them: image + text → Markdown/Text cell (image via **file upload**, not URL); **web-page/iframe embeds have NO native Hex equivalent** (text cells reject HTML/iframes) → Python `IFrame` or flag, like maps. Full mapping table in [`building-cells.md`](building-cells.md).
...
- **Hex runs cells as a dependency graph — input parameters must be UPSTREAM of their consumers.** A cell that references an input via `{{ var }}` depends on that input cell, so the **input parameter cell must come *before* (upstream of) every cell that uses it** — the consuming SQL/chart cells are *downstream*. Place input cells at the top of the notebook (or at least above the shared SQL). An input placed **after** its consumer leaves the variable unresolved and the run **ERRORs**. Same rule when assembling `cells[]` in YAML: order input cells ahead of the SQL that reads them.
- **Cannot read cell output.** `cell get` returns source; `run status` returns only COMPLETED/ERRORED/timing. Use **COMPLETED-vs-ERRORED as a boolean oracle** to validate SQL, probe schema, and check type-casts (a bogus table → ERRORED confirms the oracle works). Run one probe per question with **`hex cell run <cell_id>`** (a single cell + its deps) so a failure isolates to that cell — `hex run status <project_id> <run_id> --watch` blocks until COMPLETED/ERRORED.
  - **Type probe:** `SELECT YEAR(<col>) FROM <table> LIMIT 1` → ERRORED means `<col>` isn't a real date (see the type rule above).
  - **Row-existence probe (turn the oracle into a count check):** `SELECT 1.0/COUNT(*) FROM (<your query>)` → **ERRORED = zero rows** (divide-by-zero), COMPLETED = has rows. Lets you catch an empty result (e.g. a mis-translated relative-date window that filters everything out) even though you can't read values. Doesn't compare magnitudes — matching the source's actual totals is still the human visual-QA gate.
