...
This coding agent is **blind to what makes the build correct** — the live **warehouse schema**/data, the **Hex workspace context** (Context Studio descriptions, endorsed tables, semantic models, guides), and the **rendered result**. The notebook agent has all three (it runs in the workspace), so it owns **building in Hex** — the default path, and it spends Hex credits. This agent owns the durable IP either side of the build: **understanding the Looker source** (LookML + contract → measures/`dimension_group`s/table-calcs/filters/params) and **verifying the result**. Accuracy comes from the **gate** — which reviews whoever wrote the SQL and ties it to Looker's own numbers — not from this agent hand-writing every query.

**Priority order (say it to the customer up front):** (1) **accuracy** of SQL + visuals, (2) **similar look & feel**. The generative-app default delivers #2 (reproduces Looker's layout/styling far closer than native cells; the visual-QA loop verifies the render) while gated SQL underneath holds #1. Some features need approximation or deliberate setup — maps, custom/marketplace viz, exotic table calcs, user-attribute RLS (Jinja/RBAC) — name them early so "not pixel-identical" is never a surprise. **Cover the basis, don't gold-plate.**
...
# Files in this skill
- `SKILL.md` — this playbook (workflow spine); `reference/` — the on-demand detail docs described under **Reference docs (read on demand)** above.
- `templates/` — clone-and-override native Hex cell configs for the **fallback** hand-build (METRIC + EXPLORE bar/line/area/pie/scatter/faceted/pivot, `_filter_snippet.json`) + `semantic-model.example.yaml` (target format for the optional semantic model).
...
- **Semantic model (optional):** `hex context preview [--config-path <p>] [--base draft|latest] [--title <t>] [--description <d>]` → `preview_id`; `hex context publish <preview_id|-> [--title <t>]`. Driven by a `hex_context.config.json` with a **`semanticProjects: [{id, path}]`** array (key is `semanticProjects`, *not* `semanticModels`) and/or **`guides`**. The `id` must be an **existing** semantic project (create the empty shell in the UI first — Context Studio → Models → ⋯ → "Copy ID"; a nonexistent id → `Forbidden`). Team/Enterprise-only; synced resources are read-only in Hex. Full mapping + spec → [`reference/semantic-model.md`](reference/semantic-model.md).
