---
name: tableau-migration-hex-agent
description: >-
...
# Tableau → Hex Migration (notebook-agent build)
...
the better-equipped agent, and this is the default path (it spends Hex credits).
...
visuals first, (2) **similar look & feel** second. Some Tableau features have no
clean 1:1 in Hex (maps, LOD/detail tooltips, cosmetic styling) — name those early
so "it isn't pixel-identical" is never a surprise. Philosophy: **cover the basis,
don't gold-plate.**
...
- [`reference/build-notebook-agent.md`](reference/build-notebook-agent.md) — **the build (default):** how to write the migration brief (intent, not literal SQL), hand it to the notebook agent, surface the live URL, and iterate.
- [`reference/sql-review.md`](reference/sql-review.md) — **SQL-fidelity gate:** ledger → independent re-derivation & diff → mistake-class checklist → read-back + differential probes. Runs **post-hoc on the agent's SQL** in the default path.
- [`reference/building-cells.md`](reference/building-cells.md) — **fallback build:** this coding agent hand-builds native cells from templates (for when the notebook agent isn't available).
...
2. **Port each workbook:** resolve connection → parse XML + understand → **choose the build path (ask the customer)** → build → **SQL-fidelity gate** → run + QA → ship the guide.
...
- **Go all the way:** parse → brief → build → gate → run → **customer visually QAs** vs. the Tableau original.
- **Tune, then scale:** fold fixes (connection mapping, calc translations, brief wording, format mappings) back into this playbook *before* batching the rest.

Why: neither agent can guarantee the rendered result matches — a human check on a tiny first batch catches systematic errors before they multiply.

# Guiding the customer
- **State the priority order up front** (accuracy first, look & feel second).
- **Name the human gates:** (1) **data connection** — you'll ask when the target is ambiguous; (2) **build path** — you'll ask which build mode (below); (3) **visual QA** — neither agent can confirm the rendered layout, so they confirm fidelity on the pilot and each batch.
...
2. **Create the Hex project and inject the raw `.twb`.** The XML fits in one markdown cell (~121 KB, no chunking) — keeps the source of truth in the project:
   ...
   ```
   Keep this `.twb` cell in the notebook but **never add it to the app layout** — it's a maintainer reference.
...
7. **Run and QA.** `hex project run` (async — poll `run status`). Confirm cell usage via `hex project export` (SQL cells present, charts bound to the right dataframes, params wired). Then hand the customer the project link + the original PNGs (`scripts/tableau_shots.py`) for **visual QA** side-by-side — neither agent can render the app (it's behind login; never enter credentials), so this is the real gate. App layout via export/import if desired → [`gotchas.md`](reference/gotchas.md).
...
[
  {
    ...
    "build_mode": "A",                 // A delegate-all | B pre-built-SQL | C hand-build
    "thread_id": null,                  // notebook-agent thread (modes A/B)
...
- `reference/` — `connection-mapping.md`, `tableau-semantics.md` (understand the workbook), `build-notebook-agent.md` (the default build), `sql-review.md` (fidelity gate), `building-cells.md` (hand-build fallback), `datasource-guide.md`, `gotchas.md`.
...
- **Cells:** `hex cell create` makes only code/sql/markdown; INPUT (parameter) cells + connection-less dataframe-SQL companions are authored in YAML (fallback path). `hex cell run <id> --with-output` returns result rows; after a YAML import use the **API id from `hex cell list`**, not the export `cellId`.
