...
the migration reproduce Tableau's bespoke layout, tab navigation, and pixel styling
in a way native EXPLORE/METRIC cells can't — while the numbers stay in inspectable,
gated SQL. It pairs with the **visual-QA loop**
...
> the Tableau source* and *verifying the result*; the notebook agent still owns
...
When a worksheet's datasource is **custom SQL** (a `<relation type='text'>` query in
the `.twb`), that query **already ran against this warehouse in this dialect** — do
**not** re-translate it. Lift it verbatim into a SQL cell and validate it runs
(`hex cell run --with-output`). Re-derivation is for the VizQL/relational workbooks
(no custom SQL), where you rebuild intent per [`tableau-semantics.md`](tableau-semantics.md).
Either way the result is a native SQL cell that goes through the gate.
...
- **Metrics + formulas** — the derivation each tile reads and what it computes.
- **Filter wiring** — which inputs/params drive this tile (and cross-filter actions).
- **Colors as hex codes** — pulled from the `.twb` XML (`<style-rule>` /
  `<encoding>` palettes / `<color>` elements), **not** guessed from a screenshot.
...
brief already carries (see [`build-notebook-agent.md`](build-notebook-agent.md) §"What
the brief must contain"); the generative build just formalizes the **styling** half of it into an
extractable, diffable block because the visual loop leans on it.
...
> the full spec for migrating a Tableau dashboard into this project.*
>
...
1. **SQL-fidelity gate on the native cells** — runs on the
   inspectable SQL cells underneath the app. [`sql-review.md`](sql-review.md).
2. **Visual-QA loop on the render** — because there are no diff-able chart cells, the
   *rendered* app is how you verify look & feel. Headless screenshot →
   panel-by-panel diff vs. the source PNG → surgical fix batch → repeat.
